...
from .models import Booking, FoodOrder
...
def total_amount(request):
    """
    Returns the total amount payable for the authenticated user's bookings and food orders.
    Sums Booking.amount_required and FoodOrder.total_price().
    """
    total = 0

    if request.user.is_authenticated:
        # --- BOOKINGS ---
        bookings = Booking.objects.all()  # optionally filter by user=request.user
        booking_total = sum(booking.amount_required for booking in bookings)

        # --- FOOD ORDERS ---
        food_orders = FoodOrder.objects.all()  # optionally filter by user=request.user
        food_total = sum(order.total_price() for order in food_orders)  # <- call the method

        total = booking_total + food_total
